# 心理咨询对话系统

[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

> **基于双知识库RAG与三层记忆的智能心理咨询对话系统**

[English](README.md) | [论文]() | [文档](docs/)

---

## 📝 项目简介

本仓库是我们论文的实现代码：

**《基于双知识库RAG与三层记忆的智能心理咨询对话系统》**

系统解决了AI心理健康支持中的关键挑战：
- **专业准确性**：通过检索增强生成(RAG)提供循证支持
- **长期个性化**：通过三层记忆系统实现跨会话追踪
- **隐私保护**：支持本地模型部署

### 核心特性

🔬 **双知识库架构**
- 分离的专业心理学知识库（CBT、焦虑管理等）
- 个人用户档案知识库
- 差异化检索策略平衡专业性与个性化

🧠 **三层记忆系统**
- **工作记忆**：当前会话上下文（最近10轮对话）
- **短期记忆**：会话级别摘要（最近20个会话）
- **长期记忆**：持久化用户档案和跨会话趋势

🛠️ **模块化与灵活配置**
- 支持本地模型（Qwen2-7B）和API模型（GPT-4）
- 通过YAML文件轻松配置
- 独立模块测试和替换

---

## 🚀 快速开始

### 安装

```bash
# 克隆仓库
git clone https://github.com/yourusername/psychological_counseling_system.git
cd psychological_counseling_system

# 安装依赖
pip install -r requirements.txt

# 下载嵌入模型（首次运行）
python -c "from sentence_transformers import SentenceTransformer; SentenceTransformer('paraphrase-multilingual-mpnet-base-v2')"
```

### 基本使用

```python
from dialogue import create_dialogue_manager_from_config
import yaml

# 加载配置
with open('configs/config.yaml', 'r', encoding='utf-8') as f:
    config = yaml.safe_load(f)

# 创建对话管理器
manager = create_dialogue_manager_from_config(config)

# 开始对话
user_id = "user001"
session_id = manager.start_session(user_id)

# 聊天
response = manager.chat(
    user_id=user_id,
    session_id=session_id,
    user_message="我最近感觉很焦虑..."
)

print(response)
```

### 运行评估实验

```bash
# 快速测试（5分钟）
python evaluation/scripts/run_quick_test.py

# 完整对比实验（复现论文结果）
python examples/comparison_experiment.py
```

详见[快速开始指南](docs/quickstart.md)。

---

## 📊 系统架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         用户界面                                  │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      对话管理器                                   │
│  协调各模块 · 上下文构建 · 提示词工程                              │
└──────┬───────────────────┬────────────────────┬─────────────────┘
       │                   │                    │
       ▼                   ▼                    ▼
┌─────────────┐   ┌──────────────────┐   ┌────────────────────┐
│  LLM层      │   │   RAG层          │   │   记忆层            │
│             │   │                  │   │                    │
│ • Qwen2-7B  │   │ • 专业知识库      │   │ • 工作记忆          │
│ • GPT-4     │   │ • 个人知识库      │   │ • 短期记忆          │
│             │   │                  │   │ • 长期记忆          │
└─────────────┘   └──────────────────┘   └────────────────────┘
```

详见[架构文档](docs/architecture.md)。

---

## 📂 项目结构

```
psychological_counseling_system/
├── README.md                    # 本文件
├── INSTALLATION.md              # 详细安装指南
├── requirements.txt             # Python依赖
│
├── configs/                     # 配置文件
│   └── config.yaml              # 主配置
│
├── dialogue/                    # 对话管理模块
├── llm/                         # LLM层
├── knowledge/                   # RAG层
├── memory/                      # 记忆系统
├── evaluation/                  # 评估框架
├── examples/                    # 使用示例
├── experiments/                 # 实验结果
├── docs/                        # 文档
└── models/                      # 模型存储
```

---

## 📚 文档

- [安装指南](INSTALLATION.md) - 详细安装步骤
- [快速开始](docs/quickstart.md) - 5分钟上手
- [系统架构](docs/architecture.md) - 系统设计
- [配置说明](docs/configuration.md) - 配置参数
- [评估指南](docs/evaluation.md) - 如何复现实验
- [使用示例](docs/examples.md) - 代码示例

---

## 🔬 复现论文结果

我们的论文包含三个主要实验：

### 1. 对比实验

对比三种配置：裸LLM、LLM+RAG、完整系统

```bash
python examples/comparison_experiment.py
```

结果保存在 `experiments/` 目录。

### 2. 案例研究

运行特定咨询场景：

```bash
python examples/case_study.py --scenario anxiety
```

### 3. 用户体验评估

详见[评估指南](docs/evaluation.md)。

---

## 🛠️ 配置

通过 `configs/config.yaml` 灵活配置系统：

```yaml
llm:
  backend: 'local'  # 'local' 或 'api'
  local:
    model_path: 'models/qwen2-7b-instruct-q4_k_m.gguf'
  api:
    provider: 'openai'
    model: 'gpt-4o-mini'

rag:
  retrieval:
    top_k: 5
    score_threshold: 0.5

memory:
  layers:
    session:
      max_turns: 10
    profile:
      auto_update: true
```

详见[配置指南](docs/configuration.md)。

---

## 📄 引用

如果您在研究中使用了本代码，请引用我们的论文：

```bibtex
@article{yourname2024psychological,
  title={基于双知识库RAG与三层记忆的智能心理咨询对话系统},
  author={作者姓名},
  journal={期刊名称},
  year={2024}
}
```

---

## 📞 联系方式

- **作者**：[您的姓名]
- **邮箱**：your.email@example.com
- **问题反馈**：[GitHub Issues](https://github.com/yourusername/psychological_counseling_system/issues)

---

## 📜 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件。

---

## ⚠️ 免责声明

本系统仅用于研究目的，**不应**替代专业心理健康服务。如果您或您认识的人正在经历心理健康危机，请立即联系持证心理健康专业人员或紧急服务。

**危机资源：**
- 心理援助热线：400-161-9995
- 北京心理危机研究与干预中心：010-82951332
- 生命热线：400-821-1215
